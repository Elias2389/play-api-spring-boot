...
class RestExceptionHandler {
    ...
    fun handleExceptionMovieAlreadyExisteException(exception: Exception): ResponseEntity<Error> {
    ...
    fun handleExceptionMovieValidation(exception: MethodArgumentNotValidException): ResponseEntity<List<Error>> {
        val errors = ArrayList<Error>()
        exception.bindingResult.fieldErrors.forEach { error ->
            errors.add(Error(message = error.defaultMessage ?: "Unknown error", type = error.field))
        }
        return ResponseEntity.badRequest().body(errors)
    }

    @ExceptionHandler(Exception::class)
    fun handlerExceptions(exception: Exception): ResponseEntity<Error> {
        val error = Error(message = exception.message ?: "Unknown error", type = "unknown")
